...
class LabelParser:
    ...
    def parse_from_list(labels: list[str]) -> pd.DataFrame:
        ...
        """
        return pd.DataFrame([LabelParser.parse(label) for label in labels])
...
class SyntenyParser:
    ...
    def reformat_synteny_structure(synteny_structure: str) -> str:
        ...
        synteny_structure = (
            synteny_structure.replace(" |", "|").replace("| ", "|").strip()
        )
    ...
    def is_valid_structure(synteny_structure: str) -> bool:
        ...
        parsed_struc = SyntenyParser.parse_synteny_structure(synteny_structure)
        right_type = all(
            (
    ...
    def get_HMM_groups_in_structure(synteny_structure: str) -> list[str]:
        ...
        if not links:
            ...
            sys.exit(1)
        hmm_groups = [
            SyntenyParser.split_strand_from_locus(h)[1]
            for h in links
            if not h.isdigit()
        ]
        return hmm_groups
    ...
    def get_all_HMMs_in_structure(synteny_structure: str) -> list[str]:
        ...
        a list with all of them.
        """
        hmm_groups = SyntenyParser.get_HMM_groups_in_structure(synteny_structure)
    ...
    def get_strands_in_structure(
        ...
        if not links:
            ...
            sys.exit(1)
        return [
            SyntenyParser.split_strand_from_locus(h, parsed_symbol)[0]
            for h in links
            if not h.isdigit()
        ]
    ...
    def parse_synteny_structure(synteny_structure: str) -> dict:
        ...
        """
        max_dists = SyntenyParser.get_maximum_distances_in_structure(synteny_structure)
        hmm_groups = SyntenyParser.get_HMM_groups_in_structure(synteny_structure)
        strands = SyntenyParser.get_strands_in_structure(synteny_structure)
    ...
    def parse_genes_in_synteny_structure(
        ...
        """
        pgap = PGAP(hmm_meta)
        gene_symbols = SyntenyParser.get_gene_symbols_in_structure(synteny_structure)
        strand_locs = SyntenyParser.get_strands_in_structure(
            synteny_structure, parsed_symbol=False
        )
        gene_dists = SyntenyParser.get_maximum_distances_in_structure(synteny_structure)
        hmm_groups = {
